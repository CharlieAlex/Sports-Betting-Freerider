...
class Leaderboard:
    def __init__(self, alliance, during, page, gameday="today"):
        ...
        self.header = {
            ...
            "Referer": random.choice(back_links),
        }
    ...
    def crawl_content(self):
        board_url = (
            self.web_url
            + f"billboard/mainPrediction?during={self.during}&allianceid={self.alliance}&page={self.page}"
        )
        print("向排行榜發送爬蟲要求!")
    ...
    def board_json(self):
        ...
        results = re.search(r"var vueData = (\{.*\});", target_content)
    ...
    def dataframe(self):
        ...
        df["linkUrl"] = (
            self.user_url
            + df["userid"]
            + f"&allianceid={self.alliance}&gameday={self.gameday}"
        )
        return df
...
class Rank_user:
    ...
    def prediction(self) -> pd.DataFrame:
        def clean_table(table):
            ...
            return table[
                ["userid", "nickname", "rank", "mode", "game", "prediction", "result"]
            ]
        ...
        try:
            ...
            prediction_list = self.html_content.find_all(
                "td", class_="managerpredictcon"
            )
            ...
            if universe_tablebox.shape[0] != 0 or bank_tablebox.shape[0] != 0:
                tablebox = pd.concat(
                    [universe_tablebox, bank_tablebox], ignore_index=True
                )
            ...
            print(e)
            print(
                f"未知錯誤請查看{self.user_data.nickname}的網址: {self.user_data.linkUrl}"
            )
        return tablebox


def parse_during(during: str) -> int:
    """
    避免匹配出的結果是 None 時，無法使用 group(0) 回傳結果。
    """
    if re.match(r"^yesterday$", during):
        return 1
    else:
        match = re.search(r"\d+", during)
        if match:
            return int(match.group(0))
        else:
            raise ValueError(f"Invalid format for 'during': {during}")
...
if __name__ == "__main__":
    ...
    rank_list = Leaderboard(alliance_dict[target], during, page=1, gameday="today")
