...
class Output_maker:
    def __init__(self, leaderboard, prediction) -> None:
        ...
        self.merge_df = pd.merge(
            self.leaderboard_df, self.prediction_df, on="userid", how="inner"
        )
        self.is_mainpush = self.merge_df["main_push"]
    ...
    def combine_game(self, df: pd.DataFrame) -> pd.DataFrame:
        df["game"] = (
            ...
            .str.replace("(主)", "")
            .str.extract(
                rf"(?P<guest_team>{team_pattern})\s*(?P<home_team>{team_pattern})"
            )
    ...
    def fix_team_name(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.str.replace("費城人", "費城76人").str.replace(
            "阿德雷德人", "阿德雷德36人"
        )
        return df
    ...
    def count_prediction(self, df: pd.DataFrame) -> pd.DataFrame:
        df["count"] = 1
        return (
            df[["game", "prediction", "count"]]
            .groupby(["game", "prediction"])
            .count()
    ...
    def extend_result(self, df: pd.DataFrame) -> pd.DataFrame:
        def switch_result(df: pd.DataFrame, res: list[str]) -> pd.DataFrame:
        ...
        def count_duplicate(df: pd.DataFrame) -> pd.DataFrame:
            return (
                df.groupby(["game", "prediction"])
                .size()
                .reset_index(name="duplicate")
                .reset_index(drop=True)
            )

        is_totalover = df["prediction"].isin(["大分", "小分"])
        df_over, df_win = df[is_totalover], df[~is_totalover]
        teams = pd.Series(df_win["game"]).str.split(" vs ", expand=True)
        preds = pd.Series(df_win["prediction"]).str.split(" ", expand=True)
        ...
        df_final = (
            pd.merge(
                ...
                pd.concat(
                    [df_over.pipe(count_duplicate), df_win.pipe(count_duplicate)]
                ),
                how="left",
                on=["game", "prediction"],
            )
